...
<title>Atividades Disponíveis</title>
<link rel="stylesheet"
...
<body style="height:auto">
	...
	<div class="wrapper">
		...
		<div class="center-container">
			<h2 style="text-align: center;">Atividades Disponíveis</h2>
			...
			for (int i = 0; i < diasSemana.length; i++) {
				...
				List<ManchaPT> manchas = GestorSBD.getAtividadesIndividuais(GestorSBD.getPTIDcliente(cliente.getIDCliente()), diaSemana);
			%>
			<h4><%=diaSemana%></h4>
				...
				<tbody>
					...
					for (ManchaPT mancha : manchas) {
						Atividade atividade = GestorSBD.getAtividade(mancha.getManchaID());
						 if (!(atividade.getTipo().equals("Data") && atividade.getDataReal().toLocalDate().isBefore(LocalDate.now()))){
						if (GestorSBD.manchaTemVagas(mancha.getManchaID()) || GestorSBD.clienteRegistadoEmAtividade(atividade.getIdAti(), cliente.getIDCliente())) {
					%>
					<tr>
						...
						if (GestorSBD.clienteRegistadoEmAtividade(atividade.getIdAti(), cliente.getIDCliente())) {
						%>
						<td>Inscrito</td>
						<td>
							<form action="ClienteServlet" method="post">
								<input type="hidden" name="flag" value="cancelarInscricao">
								<input type="hidden" name="tipo" value="individual">
								<input type="hidden" name="clienteID"
									value="<%=cliente.getIDCliente()%>">
								<input type="hidden" name="idAti"
									value="<%=atividade.getIdAti()%>">
								<button type="submit" class="btn btn-primary sign-up-btn">Cancelar</button>
							</form>
						</td>
						<%
						} else {
						%>
						<td>Vaga Disponivel</td>
						<td>
							<form action="ClienteServlet" method="post">
							<input type="hidden" name="tipo" value="individual">
								<input type="hidden" name="flag" value="aderirAtividade">
								<input type="hidden" name="manchaID"
									value="<%=mancha.getManchaID()%>">
								<input type="hidden" name="clienteUsername"
									value="<%=username%>">
								<button type="submit" class="btn btn-primary sign-up-btn">Aderir</button>
							</form>
						</td>
						<%
						}
